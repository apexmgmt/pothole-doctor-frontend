...
import { Commission, Estimate } from '@/types'
...
const SalesRepresentativeCard = ({ estimateDetails, profit = 0, total = 0 }: SalesRepresentativeCardProps) => {
  const commissions = (estimateDetails?.assign_user?.userable?.commission_type?.commissions as Commission[]) ?? []
  const commissionTypeName = estimateDetails?.assign_user?.userable?.commission_type?.name

  const effectiveCommission = calculateRuleCommission(commissions, profit, total)
