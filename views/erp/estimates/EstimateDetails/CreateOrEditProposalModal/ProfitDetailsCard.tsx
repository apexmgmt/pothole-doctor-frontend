  ...
  <Card className='bg-accent/40 border-accent'>
    <CardContent className='p-4 flex flex-col h-full'>
      <div className='flex-1'>
        <h6 className='text-xs font-semibold mb-4 flex justify-end'>{'Profit'}</h6>
        <div className='flex justify-between mb-3'>
          <Badge>{profitPercent.toFixed(2)}%</Badge>
          <p className='text-xs'>{formatCurrency(profitAmount)}</p>
        </div>
      </div>
      <Separator className='mb-3' />
      {totalCost === undefined ? (
        <div className='flex justify-between'>
          <p className='text-xs font-semibold'>{totalNetProfit !== undefined ? 'Total Net Profit' : 'Total Profit'}</p>
