...
const CreateOrEditContactModal: React.FC<CreateOrEditContactModalProps> = ({
  ...
  const form = useForm<LeadContactPayload>({
    defaultValues: {
      client_id: clientId,
      name: contact?.name || '',
      email: contact?.email || '',
  ...
  const onSubmit = async (values: LeadContactPayload) => {
    try {
      if (mode === 'edit' && contact_id) {
        await LeadContactService.update(contact_id, values)
        toast.success('Contact updated successfully')
      } else {
        await LeadContactService.store(values)
