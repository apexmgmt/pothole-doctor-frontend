...
import { Column, CountryWithStates, DataTableApiResponse, LeadAddress } from '@/types'
...
const LeadAddresses = ({
  ...
  const fetchData = async () => {
    setIsLoading(true)
    try {
      LeadAddressService.index(filterOptions)
        .then(response => {
  ...
  const handleOpenEditModal = async (id: string) => {
    ...
    try {
      const response = await LeadAddressService.show(id)
  ...
  return (
    <>
      <CommonTable
        data={{
          data: (apiResponse?.data as LeadAddress[]) || [],
