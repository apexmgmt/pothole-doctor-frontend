...
import { LeadContact, LeadContactPayload } from '@/types/leads/leads_contacts.types'
import { CountryWithStates, LeadAddress, LeadAddressPayload } from '@/types'
import LeadAddressService from '@/services/api/leads/lead-addresses.service'
...
const CreateOrEditAddressModal: React.FC<CreateOrEditAddressModalProps> = ({
  ...
  const form = useForm<LeadAddressPayload>({
    defaultValues: {
      client_id: clientId,
      title: address?.title || '',
  ...
  const onSubmit = async (values: LeadAddressPayload) => {
    try {
      // Remove country_id from payload
      const { country_id, ...payload } = values
      if (mode === 'edit' && address_id) {
        await LeadAddressService.update(address_id, payload)
        toast.success('Address updated successfully')
      } else {
        await LeadAddressService.store(payload)
