...
interface CreateOrEditWorkOrderDocumentModalProps {
  mode?: 'create' | 'edit'
  open: boolean
...
const CreateOrEditWorkOrderDocumentModal = ({
  mode = 'create',
  open,
  onOpenChange,
  workOrderId,
  ...
  documentDetails,
  onSuccess
}: CreateOrEditWorkOrderDocumentModalProps) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      if (mode === 'create') {
        await WorkOrderDocumentService.store(formData)
        toast.success('Document added successfully')
        form.reset()
        onOpenChange(false)
        onSuccess?.()
      } else if (mode === 'edit' && documentId) {
        await WorkOrderDocumentService.update(documentId, formData)
  ...
  return (
    <CommonDialog
      ...
      description={mode === 'create' ? 'Upload a document for this work order' : 'Replace the existing document'}
      maxWidth='sm'
...
export default CreateOrEditWorkOrderDocumentModal
