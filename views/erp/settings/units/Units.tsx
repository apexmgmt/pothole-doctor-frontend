...
interface UnitsProps {
  group?: string | 'uom' | 'measure'
  initialData?: DataTableApiResponse<Unit> | null
  permissions?: {
...
const Units: React.FC<UnitsProps> = ({ group, initialData, permissions }) => {
  const router = useRouter()
  const dispatch = useAppDispatch()
  const searchParams = useSearchParams()

  const [apiResponse, setApiResponse] = useState<DataTableApiResponse<Unit> | null>(initialData || null)
  const [isLoading, setIsLoading] = useState<boolean>(false)
  const [searchValue, setSearchValue] = useState<string>('')
  const [inlineMode, setInlineMode] = useState<'create' | 'edit' | null>(null)
  const [editingUnitId, setEditingUnitId] = useState<string | null>(null)

  const canCreateUnit = permissions?.canCreateUnit ?? false
  const canEditUnit = permissions?.canEditUnit ?? false
  const canDeleteUnit = permissions?.canDeleteUnit ?? false
