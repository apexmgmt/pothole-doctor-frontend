...
export default function ServicesSection() {
  ...
  const services: Service[] = [
    ...
    },
    {
      ...
      description: 'Our team ensures proper drainage and stability with expert catch basin restoration and rebuilding.',
      ...
      <div className='container'>
        ...
        <div className='grid lg:grid-rows-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xl:gap-6 '>
          ...
          <div className='sm:col-span-2 row-span-2 relative rounded-lg overflow-hidden group'>
            <div>
              <Image fill src={services[0].image} alt='' className='h-full w-full !relative object-cover' />
            </div>
            <div className='absolute bottom-0 left-0 right-0 p-4 sm:p-5 md:p-[30px] text-white'>
              <h3 className='text-lg sm:text-[28px] font-semibold mb-4 font-primary'>{services[0].title}</h3>
              <p className='text-sm sm:text-base text-[#EEEEEE]'>{services[0].description}</p>
            </div>
          </div>
          ...
          {services.slice(1).map(service => (
            ...
            >
              <div>
                <Image fill src={service.image} alt='' className='h-full w-full !relative object-cover' />
              </div>
              ...
              <div className='absolute bottom-0 left-0 right-0 p-4 xl:p-5 text-white'>
                ...
                >
                  <p className='text-sm xl:text-base text-[#EEEEEE]'>{service.description}</p>
                </div>
              </div>
            </div>
          ))}
